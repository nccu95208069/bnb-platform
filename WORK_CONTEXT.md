...
The owner requests comparing check-ins from 30 calendar days before today onward, with no future upper bound, using Asia/Taipei dates. On 2026-09-06 the inclusive cutoff is 2026-08-07. Also include earlier check-ins that have not checked out. This is a future synchronization scope, not a calendar browsing limit or permission to delete historical data. Preserve out-of-scope records and their unresolved historical acknowledgements. A tracked row missing from the filtered result must be checked by source-scoped J identity before interpreting it as deletion: it may have moved outside the window. Retire issue acknowledgements only after verified resolution, not merely aging out. See the source pilot document for reconciliation safeguards. Automatic synchronization remains disabled.

## Sheet monitor implementation — 2026-09-06

Owner requested implementation. The local frontend now contains a deterministic read-only Google connector, scoped reconciliation, two-observation confirmation, private compressed Redis state, expiring fenced lock, verified atomic publication, protected operator/cron/status routes, bounded audit and human-readable sync health with browser refresh. Synthetic tests and a real read-only Sheet connection pass. No source writes or Google watch subscription are involved. Production activation is pending first-time Upstash terms acceptance, provisioning/configuration and actual scheduler/deployment verification; do not claim the public site is already live-synced. Details and activation/rollback are in `docs/work-handoff/SHEET_MONITOR.md`.
