...
**Activation:** owner explicitly confirmed the private-login scope on 2026-09-06; the two production secret settings are now enabled. Anonymous/login/named-read/logout checks passed against the production alias for both sources, and mobile two-line month names were verified in the browser. Public responses still contain no real names. Employee access remains unimplemented; never copy the owner's private entry credential into this document.

### Owner-selected password

The authenticated `/calendar-password` page lets the owner replace the generated entry code with a chosen password. Credentials are persisted in the existing private Redis service under `sweetfun-os:owner-auth:v1:credential` (scrypt salt/hash/revision only), separate from booking monitor state. The production record is initialized once from the existing code hash using SET NX; it must not be overwritten by deployment scripts. Missing or corrupt state must fail closed. Changes require a valid current session, and after initial setup also the current password; compare-and-set plus re-read verifies the write. Old code and session signatures stop working after the revision changes. Do not use the production credential record for tests; use a unique temporary namespace and remove only those test keys.
