...
## Conversation-first UI — 2026-09-26

Owner supersedes previous three-column design: use a very simple LINE-like contact list and chat bubbles. Code `46137e5`; staged deployment `dpl_4Xu2bJKTGhYCC3PeGefW4EYWFYS8`. Persistent auth, role ceilings and data rules unchanged. Tools/files/history/setup live behind the top-right menu; reports initially collapsed; onboarding is optional and compact, not a forced long page. Mobile contact switching, menu tools, report expand and customer disconnected state visually verified at 390x844; production build passed. Lint zero errors (unused legacy detailsHTML plus two pre-existing warnings).

Customer service coordination with task `01a0dbb2-0e20-7140-ab0e-890fab87df37`: stable agent id `bnb-customer-service`, name 民宿客服. That task owns host runtime and OA integration, separate checkout; this task owns /bots UI/deployments. The current contact is an explicitly disconnected placeholder, not an operational agent. Host status and Messaging API status must be separate. Real OA tags/names must not be confused with internal metadata. No LINE messages sent or credentials configured in this UI update.
