...
export function FinanceSummary(){
 ...
 <details className="mt-3 text-sm"><summary className="cursor-pointer">{t('紀錄與來源')}</summary><p className="mt-2 break-all text-xs">OS: {o.id}</p><p className="text-xs">{t('來源列數')} {o.row_ids.length} · {t('舊付款標記')} {o.legacy_status.split('/').map(s=>t(({paid:'已付清',unpaid:'未付款',deposit:'部分付款',unknown:'待確認'} as Record<string,string>)[s]??'待確認')).join(' / ')}</p>{o.records.length?<ul className="mt-2 space-y-1">{o.records.map(r=><li key={r.id}>{r.at} · {money(r.amount_cents)} · {t(METHODS[r.method as keyof typeof METHODS]??r.method)} · {r.actor} · {t(r.source==='calendar'?'日曆':'記帳')}</li>)}</ul>:<p>{t('尚無紀錄')}</p>}</details>
