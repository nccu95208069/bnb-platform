...
function ReceiptForm({report,order,onClose,onSaved}:{report:FinanceReport;order:FinanceOrder;onClose:()=>void;onSaved:()=>void}){
 ...
 const [amount,setAmount]=useState(''),[date,setDate]=useState(taipeiDate()),[method,setMethod]=useState('bank_transfer'),[note,setNote]=useState('');
 const input='mt-1 w-full rounded-lg border bg-white px-3 py-2.5';
 return <Dialog open onOpenChange={open=>!open&&!busy&&!pending&&onClose()}><DialogContent showCloseButton={!busy&&!pending}><DialogTitle>{t('登記收款')}</DialogTitle><DialogDescription>{order.rooms.join('、')} · {order.check_in.replaceAll('-','/')}<br/>{t('只登記旅宿實際收到的款項，請款申請不算收款。')}</DialogDescription>
