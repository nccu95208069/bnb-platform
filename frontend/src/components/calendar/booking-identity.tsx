...
export function BookingIdentity({ booking }: { booking: CalendarBooking }) {
  ...
  return (
    <span className="min-w-0 flex-1">
      ...
      <span className="font-semibold">{booking.source_conflict ? uiText("待核對") : uiText(PLATFORM_LABELS[booking.platform] ?? "其他")}</span>
...
export function GuestRemarks({booking, compact = false, limit = 1}: {booking:CalendarBooking;compact?:boolean;limit?:number}) {
 ...
 return <span aria-label={uiText("重點備註：{0}", [remarks.map(r=>r.label).join('、')])} className={compact?'mt-0.5 flex min-w-0 items-center gap-0.5 text-[9px] leading-[12px]':'mt-1 flex flex-wrap gap-1 text-xs leading-5'}>
