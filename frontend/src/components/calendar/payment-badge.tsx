...
export function PaymentBadge({booking,compact=false}:{booking:CalendarBooking;compact?:boolean}) {
  ...
  return <span title={PAYMENT_LABELS[booking.payment_status]} aria-label={PAYMENT_LABELS[booking.payment_status]} className={`inline-flex shrink-0 items-center justify-center rounded-sm border px-0.5 text-[9px] leading-3 font-semibold ${styles[booking.payment_status]}`}>{compact?symbols[booking.payment_status]:PAYMENT_LABELS[booking.payment_status]}</span>;
}
