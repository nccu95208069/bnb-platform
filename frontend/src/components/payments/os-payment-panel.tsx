...
const uiLocale = useIntlLocale();
  ...
  return <section className="rounded-xl border p-3 space-y-3" aria-label={uiText("OS 付款紀錄")}>
    <h3 className="font-semibold">{uiText("付款紀錄")}</h3>
    <p className="text-xs text-muted-foreground">{uiText("存於 OS，不會被訂房表同步覆蓋。此處記錄客人付款，不代表 OTA 已撥款到旅宿。")}</p>
    {error&&<p role="alert" className="text-sm text-red-700">{uiText(error)}</p>}
    {success&&<p role="status" className="text-sm text-green-700">{uiText(success)}</p>}
    {!check&&!error&&<p>{uiText("讀取付款紀錄…")}</p>}
    {!check&&error&&<Button variant="outline" onClick={()=>setReload(v=>v+1)}>{uiText("重新載入")}</Button>}
    {check&&<>
      <p className="text-sm">{uiText(PAYMENT_LABELS[check.payment_status])} {uiText("· 整張訂單")}{check.rooms.join('、')} · {check.nights} {uiText("房晚 · 房費")}{formatMoney(check.total, uiLocale)}</p>
      {check.source_paid&&<p className="text-xs text-muted-foreground">{uiText("訂房表已標記付清，歷史收款金額與時間尚未提供。")}</p>}
      <p className="text-xs">{uiText("OS 已登記房費：")}{formatMoney(check.ledger.receipts.filter(r=>r.payment_type!=='other').reduce((s,r)=>s+r.amount,0), uiLocale)} {uiText("／ 其他費用：")}{formatMoney(check.ledger.receipts.filter(r=>r.payment_type==='other').reduce((s,r)=>s+r.amount,0), uiLocale)}</p>
      {!!check.finance_received&&<p className="text-xs">{uiText("財務已登記房費：")}{formatMoney(check.finance_received, uiLocale)}{uiText("（已計入付款狀態，請勿重複登記）")}</p>}
      {canRecord&&check.can_record&&!type&&<div className="flex flex-wrap gap-2">{(['deposit','balance','other','full'] as PaymentType[]).map(t=><Button key={t} size="sm" variant="outline" onClick={()=>{setType(t);setAmount(t==='full'?String(Math.max(0,Math.round((check.total-(check.finance_received??0)-check.ledger.receipts.filter(r=>r.payment_type!=='other').reduce((s,r)=>s+r.amount,0))*100)/100)):'');setAt(nowTaipei());setNote('');setConfirmed(false);setSuccess('');}}>{uiText("登記")}{uiText(PAYMENT_TYPE_LABELS[t])}</Button>)}</div>}
      {type&&<form className="space-y-3 border-t pt-3" onSubmit={e=>{e.preventDefault();void save();}}>
        <h4 className="font-medium">{uiText("登記")}{uiText(PAYMENT_TYPE_LABELS[type])}</h4>
        <fieldset disabled={busy||!!pending} className="space-y-3 disabled:opacity-60">
          <label className="block text-sm">{uiText("本次實收金額（元）")}<input required type="number" min="0.01" max="10000000" step="0.01" value={amount} onChange={e=>setAmount(e.target.value)} className="mt-1 block w-full rounded border p-2" /></label>
          <label className="block text-sm">{uiText("付款方式")}<select value={method} onChange={e=>setMethod(e.target.value as PaymentMethod)} className="mt-1 block w-full rounded border p-2">{Object.entries(PAYMENT_METHOD_LABELS).map(([k,v])=><option key={k} value={k}>{uiText(v)}</option>)}</select></label>
          <label className="block text-sm">{uiText("收款日期與時間（台灣時間）")}<input required type="datetime-local" value={at} onChange={e=>setAt(e.target.value)} className="mt-1 block w-full min-w-0 rounded border p-2" /></label>
          <label className="block text-sm">{uiText("備註")}<input maxLength={500} value={note} onChange={e=>setNote(e.target.value)} className="mt-1 block w-full rounded border p-2" /></label>
          {(type==='balance'||type==='full')&&<label className="flex gap-2 text-sm"><input required type="checkbox" checked={confirmed} onChange={e=>setConfirmed(e.target.checked)} />{uiText("確認這張訂單所有房間、所有晚數的房費已付清（含之前付款）。")}</label>}
          {type==='other'&&<p className="text-xs">{uiText("其他費用獨立記錄，不會把房費改成已付清。")}</p>}
        </fieldset>
        <div className="flex gap-2"><Button disabled={busy} type="submit">{busy?uiText("確認儲存中…"):pending?uiText("重試並確認原付款"):uiText("確認登記")}</Button>{!pending&&<Button type="button" variant="outline" disabled={busy} onClick={()=>setType(null)}>{uiText("取消")}</Button>}</div>
        {pending&&!busy&&<p className="text-xs">{uiText("結果尚未確認，重試會核對原付款，不會重複登記。")}</p>}
      </form>}
      {!check.ledger.receipts.length&&<p className="text-sm text-muted-foreground">{uiText("OS 尚無付款明細。")}</p>}
      <ul className="divide-y">{[...check.ledger.receipts].reverse().map(r=><li key={r.id} className="py-2 text-sm">
        <div className="font-medium">{uiText(PAYMENT_TYPE_LABELS[r.payment_type])} {formatMoney(r.amount, uiLocale)} · {uiText(PAYMENT_METHOD_LABELS[r.payment_method])}</div>
        <div>{uiText("收款")}{new Date(r.received_at).toLocaleString(uiLocale,{timeZone:'Asia/Taipei',hour12:false})}</div>
        <div className="text-xs text-muted-foreground">{r.actor_name} {uiText("登記於")}{new Date(r.created_at).toLocaleString(uiLocale,{timeZone:'Asia/Taipei',hour12:false})}</div>
