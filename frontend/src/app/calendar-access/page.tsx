...
export default function CalendarAccessPage() {
  ...
  return <main className="flex min-h-dvh items-center justify-center bg-slate-50 px-5">
    <form onSubmit={enter} className="w-full max-w-sm space-y-5 rounded-2xl border bg-white p-6 shadow-sm">
      ...
      <label className="block text-sm font-medium">私人密碼或登入碼<input aria-label="私人密碼或登入碼" type="password" autoComplete="current-password" required value={code} onChange={e => setCode(e.target.value)} className="mt-2 block w-full rounded-lg border px-3 py-2.5" /></label>
