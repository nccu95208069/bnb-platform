...
export async function mailStatus(){const value=await storedMail();return {configured:Boolean(value?.secret),sender:ADMIN_EMAIL,verifiedAt:value?.verifiedAt??null};}
...
export async function sendInvitation(to:string,token:string) {
  ...
  try {
    const result=await smtp.sendMail({from:{name:'Sweetfun OS',address:ADMIN_EMAIL},to,subject:'Sweetfun OS｜設定你的帳號密碼',text:`管理者邀請你使用 Sweetfun OS。\n\n請於 24 小時內開啟以下連結，自行設定密碼：\n${url}\n\n設定完成後，以本信箱及你設定的密碼登入：\nhttps://sweetfun-os.vercel.app/calendar-access\n\n本連結只能使用一次；若你未預期收到邀請，可忽略本信。管理者不會知道你設定的密碼。`});
    if(!result.accepted?.length)throw new Error('MAIL_FAILED');
  } finally {smtp.close();}
}
