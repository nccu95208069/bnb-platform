...
export async function setRecoveryPassword(input:Record<string,unknown>){
 ...
 for(const member of state.value.members){
  if(member.mustResetPassword){members.push({...member,credential:await createPasswordCredential(password),version:member.version+1});updatedCount++;}
  else members.push(member);
 }
 const next=JSON.stringify(nextWorkspace(state.value,members,'shared_recovery_password_changed','workspace'));
