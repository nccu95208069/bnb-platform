import type {FinanceAuditEvent} from './finance-audit';
...
export async function financeContext(source:SheetSourceDefinition,year:number,includeAudit=false){
 ...
 const audit:FinanceAuditEvent[]=includeAudit?[...states.flatMap(s=>s.state.operations.map(o=>o.event??{schema_version:1 as const,id:o.id,request_id:o.id,property_id:source.property.id,action:'legacy_unknown',actor_id:o.actor,actor_name:null,actor_email:null,actor_role:null,at:o.at,source:'finance' as const,result:'succeeded' as const,target_type:'legacy',target_id:o.entry_id,before:null,after:null,changed_fields:[],version_before:null,version_after:null,source_version:null,completeness:'legacy_partial' as const})),...calendar.flatMap(e=>e.audit_event?[e.audit_event]:[])]:[];
 const entries=[...states.flatMap((s,i)=>s.state.entries.map(e=>({...e,ledger_year:sorted[i]}))),...calendar.map(e=>{const {audit_event,...entry}=e;void audit_event;return entry;})];
