...
export function applyFinance(state:FinanceState,input:Record<string,unknown>,actor:Principal,property:string,year:number,now=new Date().toISOString()) {
 ...
 const hash=createHash('sha256').update(JSON.stringify([actor.id,input.action,input.kind,input.category,input.amount,input.date,input.description,input.method,input.stage,input.entry_id,input.reason,input.allocations,input.platform,input.recurrence_id,input.occurrence,input.start,input.end,input.day,input.rule_id,input.mode,input.offset])).digest('hex');
