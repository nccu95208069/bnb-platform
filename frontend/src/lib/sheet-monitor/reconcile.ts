...
import { adaptSweetfunSheet, type BookingSourceSnapshot } from "../booking-sources/sweetfun-sheet.ts";
...
export const HEADERS = ["房型", "預定人姓名", "預定平台", "入住日期", "退房日期", "預訂日期", "房費", "全額支付狀態", "檢查狀態", "唯一ID", "備註", "訂單編號"];
export const SOURCE_ID = "sweetfun-operations-sheet-v1";
...
export function normalizeRows(values: unknown[][]): SourceRow[] {
  const headers = (values[0] ?? []).map(v => String(v).trim());
  for (const h of HEADERS.filter(h => h !== "備註")) {
...
export function scopedRows(previous: SourceRow[], incoming: SourceRow[], now: string): SourceRow[] {
  ...
  while (expanded) {
    ...
    const unknownDates = new Set([...selected].filter(r => !/^(101|102|201|202|301|302)$/.test(r.cells[0])).map(r => r.cells[3]));
    for (const r of all) {
      if (!selected.has(r) && (ids.has(r.cells[9]) || (r.cells[11] && parents.has(r.cells[11])) || slots.has(`${r.cells[0]}:${r.cells[3]}`) || unknownDates.has(r.cells[3]) || (dates.has(r.cells[3]) && !/^(101|102|201|202|301|302)$/.test(r.cells[0])))) {
...
export function reconcile(state: MonitorState, values: unknown[][], now: string): MonitorState {
  const incoming = normalizeRows(values);
  ...
  const rows = state.rows ? scopedRows(state.rows, incoming, now) : incoming;
  const digest = contentDigest(rows);
  const base = { ...state, checkedAt: now, error: null };
  if (state.rows && digest === contentDigest(state.rows)) {
    const refreshed = adaptSweetfunSheet([HEADERS, ...rows.map(r => r.cells)], SOURCE_ID, state.snapshot.source.observed_at,
      state.snapshot.issues.filter(i => i.acknowledged).map(i => i.fingerprint), rows.map(r => r.sourceRow));
  ...
  const snapshot = adaptSweetfunSheet([HEADERS, ...rows.map(r => r.cells)], SOURCE_ID, now, acks, rows.map(r => r.sourceRow));
  ...
  snapshot.source.snapshot_version = hash(digest + snapshot.source.adapter_version + JSON.stringify(archived) + JSON.stringify(snapshot.issues.filter(i => i.acknowledged).map(i => i.fingerprint).sort())).slice(0, 20);
