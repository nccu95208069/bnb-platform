...
export function safeError(error: unknown): string {
  ...
  return /^(MONITOR_(STORAGE_CONFIG|STORAGE_INVALID|STORAGE_UNAVAILABLE|STORAGE_SIZE_LIMIT|GOOGLE_CONFIG|GOOGLE_AUTH|GOOGLE_READ)|SHEET_(SCHEMA_MISMATCH|MISSING_ROW_ID|INVALID_DATE|INVALID_ROOM_NIGHT|EMPTY_SOURCE|IDENTITY_MISMATCH|GRID_UNSUPPORTED|INCOMPLETE_READ))$/.test(message) ? message : "MONITOR_CHECK_FAILED";
}
export async function runMonitor(deps: { store: MonitorStore; seed: () => Promise<BookingSourceSnapshot>; read: () => Promise<unknown[][]>; now?: () => string }) {
  ...
  try {
    ...
    let next;
    try { next = reconcile(current, await deps.read(), now()); }
