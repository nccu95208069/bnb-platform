...
export type FinanceEntry = { id:string; ledger_year?:number; updated_at?:string; updated_by?:string; history?:{at:string;actor:string;actor_id:string;before:Omit<FinanceEntry,'history'>}[]; property_id:string; kind:FinanceKind; category:string;category_name?:string; amount_cents:number; date:string; description:string; method:keyof typeof METHODS; stage?:string; source:'manual'|'calendar'; actor:string; created_at:string; status:'active'|'void'; payment_account?:Pick<PaymentAccount,'id'|'name'|'method'|'last_digits'>; expense_spread?:ExpenseShare[]; advanced_by?:ExpenseAdvance; void_reason?:string; void_at?:string; void_actor?:string; order_id?:string; order_key?:string; platform?:string; allocations?:{order_id:string;amount_cents:number}[]; recurrence_id?:string; occurrence?:string };
