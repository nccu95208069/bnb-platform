...
export type FinanceEntry = { id:string; property_id:string; kind:FinanceKind; category:string;category_name?:string; amount_cents:number; date:string; description:string; method:keyof typeof METHODS; stage?:string; source:'manual'|'calendar'; actor:string; created_at:string; status:'active'|'void'; expense_spread?:ExpenseShare[]; advanced_by?:ExpenseAdvance; void_reason?:string; void_at?:string; void_actor?:string; order_id?:string; order_key?:string; platform?:string; allocations?:{order_id:string;amount_cents:number}[]; recurrence_id?:string; occurrence?:string };
export type FinanceReport = { expense_categories?:Record<string,string>; advance_accounts?:{id:string;name:string}[]; properties:{id:string;name:string}[]; property_id:string; year:number; version:number; entries:FinanceEntry[]; updated_at:string; versions:Record<string,number>; projection_version:string; orders:FinanceOrder[]; recurring:RecurringExpense[]; due:DueExpense[]; payout_rules:PayoutRule[]; excluded_orders:number };
