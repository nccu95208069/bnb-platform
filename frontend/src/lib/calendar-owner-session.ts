...
function sign(payload: string): string {
  return createHmac("sha256", `${process.env.CALENDAR_OWNER_SESSION_SECRET}:${process.env.CALENDAR_OWNER_CODE_HASH}`).update(payload).digest("base64url");
}
...
export function validOwnerSession(cookie: string | undefined, now = Date.now()): boolean {
