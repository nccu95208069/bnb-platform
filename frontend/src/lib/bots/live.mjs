...
export function liveProjection(snapshot){
 return {rows:snapshot.bookings.map(b=>({room:b.room_number,day:b.check_in,channel:b.platform,fee:b.source_conflict||b.price_hidden?null:b.room_rate,ambiguous:!!b.source_conflict})),rooms:['101','102','201','202','301','302'],asof:snapshot.source.sync.last_checked_at,hash:snapshot.source.snapshot_version};
}
export function liveQuery(snapshot,c,tool,r,a){
 ...
 const source={title:'水芳目前訂單・唯讀',asof:snapshot.source.sync.last_checked_at,version:snapshot.source.snapshot_version};
 ...
 if(tool==='availability')return {kind:'table',title:'表載房況（非可售庫存）',range:r,source,note,rows:['101','102','201','202','301','302'].map(room=>({房間:room,期間房況:bookings.some(b=>b.room_number===room&&b.source_conflict)?'有待核對房晚':bookings.some(b=>b.room_number===room)?'有已登記房晚':'未見登記房晚；可售狀態待確認'}))};
