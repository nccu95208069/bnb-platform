...
export function authorizedHistory(messages,version,dataset){return dataset!=='sandbox'?[]:messages.filter(m=>m.policyVersion===version).slice(-8).map(m=>({role:m.role,text:m.text,results:m.cards||[]}));}
...
export async function runAgent(s,c,instruction,history=[],planner=geminiPlan,depth=0){
 ...
 let p;try{p=await planner(c,instruction,c.dataset!=='sandbox'?[]:history,c.dataset!=='sandbox'?[]:trace.map(x=>({tool:x.tool,result:x.result})));}catch(e){if(cards.length)return {message:'部分工具已完成；後续處理未完成，請查看結果與錯誤。',cards,trace,error:{code:e.code||'error',message:e instanceof Fault?e.message:'模型連線暫時失敗。'},usage};throw e;}
 ...
 if(c.dataset!=='sandbox')return {message:'已依唯讀權限完成。正式來源未被修改。',cards,trace,usage};
