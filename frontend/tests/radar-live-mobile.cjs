...
(async()=>{
  ...
  try{
    ...
    const accepted=agoda.observations.filter(d=>d.identityVerified&&d.dateVerified&&d.rooms.some(r=>r.availability==='available'&&r.amount>0&&r.currency&&r.priceBasis==='tax_inclusive_stay_total'));
    console.log('Real Agoda accepted dates:',accepted.length,'of',agoda.requestedDays);
    assert.ok(accepted.length>0,'at least one real verified tax-inclusive stay total is required; sold-out status alone does not pass');
    ...
    assert.ok(accepted.some(d=>d.rooms.some(r=>r.amount>0)));
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
