...
if (!project) project = await api('/v10/projects', 'POST', { name: projectName, framework: 'nextjs', nodeVersion: '22.x' });
...
// Only this newly dedicated public-safe test project is made externally testable.
...
const result = spawnSync('/tmp/radar-deploy-tools/node_modules/.bin/vercel', args, { cwd: directory, encoding: 'utf8', timeout: 480000, maxBuffer: 8 * 1024 * 1024 });
if (result.stderr) process.stderr.write(result.stderr.replaceAll(token, '[REDACTED]'));
if (result.status !== 0) throw new Error(`Test deployment failed: exit ${result.status}`);
const urls = (result.stdout ?? '').match(/https:\/\/[a-z0-9.-]+\.vercel\.app/g) ?? [];
